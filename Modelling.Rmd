...
```{r}
write.csv(data, file = "Final_dataset.csv", row.names = FALSE)
```
...
# Model Comparision and Evaluation(using prediction)

## Compare models using LOO creiation

```{r}
#may crash
loo(model_flat, model_horse1)
```
...
test_all_hypothesis <- function(model, condition) {
  ...
  hypothesis_res <- as.da ta.frame(matrix(nrow = 0, ncol = 8))
